...
struct
  ...
  let new_server_connection t params id pushf =
    ...
    let options = Options.MSS (Ip.mtu t.ip - Wire_structs.Tcp_wire.sizeof_tcp) :: opts in
  ...
  let connect t ~dest_ip ~dest_port =
    ...
    let rx_wnd_scaleoffer = wscale_default in
    let options =
      Options.MSS (Ip.mtu t.ip - Wire_structs.Tcp_wire.sizeof_tcp) :: Options.Window_size_shift rx_wnd_scaleoffer :: []
    in
    let window = 5840 in
